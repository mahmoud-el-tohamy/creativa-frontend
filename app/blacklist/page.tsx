...
export default function BlacklistPage() {
  ...
  const {
    ...
    mutate,
    isLoading: loading,
  } = useSWR("/api/blacklist", fetcher, {
    revalidateOnFocus: true,
    onError: () => {
      showToast("حدث خطأ أثناء تحميل البيانات.", "error");
    },
  });
  const entries: BlacklistEntry[] = useMemo(
    () => entriesData || [],
    [entriesData],
  );
  ...
  const handleAddSubmit = async (
    ...
    if (!nameRegex.test(name.trim())) {
      showToast(
        "الاسم يجب أن يحتوي على حروف عربية أو إنجليزية فقط ولا يمكن أن يحتوي على أرقام.",
        "error",
      );
      return;
    }
    ...
    if (!nationalIdRegex.test(nationalId.trim())) {
      showToast(
        "الرقم القومي يجب أن يكون 14 رقماً فقط دون أي حروف أو رموز.",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      const msg =
        (error as { response?: { data?: { message?: string } } })?.response
          ?.data?.message || "حدث خطأ أثناء الإضافة.";
      showToast(msg, "error");
    } finally {
  ...
  const filteredEntries = useMemo(() => {
    let result = entries.filter((e) => {
      if (activeTab === "blacklisted" && e.status !== "blacklisted")
        return false;
    ...
    if (filters.status !== "all") {
      ...
      if (filters.status === "expiring") {
        result = result.filter((e) => {
          ...
          return d <= threeMonthsAgo && d >= fourMonthsAgo;
        });
  ...
  const selectedVisibleIds = filteredEntries
    .map((entry) => entry.id)
    .filter(
      (id): id is string => id !== undefined && validSelectedIds.includes(id),
    );
  ...
  const toggleSelectAll = () => {
    ...
    if (
      ...
      selectedVisibleIds.length === filteredIds.length
    ) {
  ...
  return (
    <RouteGuard allowedRoles={["admin", "employee", "viewer"]}>
      <main className="flex-1 bg-transparent dark:bg-transparent text-gray-900 dark:text-gray-100 p-6 sm:p-12 font-sans overflow-x-hidden">
        ...
        {toast && (
          <div
            className={`fixed top-4 left-1/2 transform -translate-x-1/2 px-6 py-3 rounded-lg shadow-lg z-50 transition-all ${toast.type === "success" ? "bg-green-600 text-white" : "bg-red-600 text-white"}`}
          >
            {toast.message}
          </div>
        )}
            ...
            {loading ? (
                ...
                <table className="w-full text-sm text-right text-gray-600">
                      ...
                      {canWrite && (
                        <th className="px-6 py-5 w-12 text-center">
                          <div className="relative inline-flex items-center justify-center cursor-pointer">
                            ...
                            />
                            <svg
                              className="absolute w-3.5 h-3.5 text-white pointer-events-none opacity-0 peer-checked:opacity-100 peer-checked:scale-100 scale-50 transition-all duration-200"
                              fill="none"
                              viewBox="0 0 24 24"
                              stroke="currentColor"
                              strokeWidth={3}
                  ...
                  <tbody>
                    {paginatedEntries.map((person) => (
                      <tr
                        key={person.id}
                        className="border-b last:border-0 hover:bg-gray-50/50 dark:hover:bg-gray-900/20 transition-colors"
                      >
                        {canWrite && (
                        ...
                        {activeTab === "warnings" ? (
                          ...
                          <>
                            ...
                            <td
                              className="px-6 py-4 text-center text-xs text-gray-500 max-w-[150px] truncate"
                              title={person.absences
                                ?.map((a) => a.track)
                                .join(", ")}
                            >
              ...
              <div className="p-16 text-center">
                ...
                <p className="text-gray-500 dark:text-gray-400">
                  ...
                  filters.dateFrom ||
                  filters.dateTo
                    ? "جرب البحث باسم أو رقم قومي مختلف أو قم بإعادة ضبط الفلاتر."
                    : "لم يتم إدراج أي شخص في قائمة المستبعدين حالياً."}
                </p>
              </div>
            )}
          </div>
