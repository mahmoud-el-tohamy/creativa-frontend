...
export default function Home() {
    ...
    try {
      ...
      if (validToBlacklist.length > 0 || attendedPeople.length > 0) {
        const res = await addManyToBlacklist(
          ...
          attendedPeople.map(p => p.nationalId),
          selectedTrack
        );
        added = res.added;
        cleared = res.cleared;
  ...
  const handleDownloadBlacklisted = () => {
    ...
    const data: (string | number)[][] = [
      ...
      ...result.absentees.map(p => [p.name, p.nationalId, "تم الإضافة للإنذارات"]),
      ...(result.invalidEntries || []).map(p => [p.name, p.nationalId, p.reason]),
      ...result.skippedExisting.map(p => [p.name, p.nationalId, "موجود مسبقاً (تم تحديثه)"])
    ];
    downloadStyledExcel({
      data,
    ...
    <main className="flex-1 bg-transparent dark:bg-transparent text-gray-900 dark:text-gray-100 p-6 sm:p-12 font-sans w-full relative min-h-screen">
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-6 mt-10">
          <div className="flex items-center gap-4 w-full sm:w-auto sm:w-80">
            <TrackSelector 
          ...
          <button
            ...
            disabled={loading}
            className={`px-10 py-4 rounded-xl text-lg font-bold text-white shadow-xl transition-all duration-300 w-full sm:w-auto min-w-[250px]
        ...
        {result && (
          <div className="mt-12 bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 p-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
              ...
              <StatCard title="إنذارات جديدة" value={result.addedCount} color="text-amber-600" bgColor="bg-amber-50" subtitle="(للمتغيبين)" />
              <StatCard title="مضافين للبلاك ليست" value={result.upgradedCount} color="text-red-600" bgColor="bg-red-50" subtitle="(تجاوزوا الغياب)" />
              <StatCard title="حاضرون غير مسجلين" value={result.unregisteredAttendees.length} color="text-purple-600" bgColor="bg-purple-50" subtitle="(حضروا بدون تسجيل)" />
            </div>
